...
class LaneCenterPlotter:
    ...
    def update_plot(self, frame, lane_center, image_center):
        ...
        # 保持x轴的范围固定
        # 当帧数超过max_frame时,滚动更新x轴的范围(只显示最近max_frame帧)
        if len(self.x_data) > self.max_frames:
            self.ax.set_xlim(self.x_data[-self.max_frames], self.x_data[-1])
            self.ax.figure.canvas.draw()  # 强制刷新画布
        plt.pause(0.01)  # 短暂延时,确保图像正常显示,不卡顿


def yolo_detect(image):
    results = model(image, verbose=False)
    person_Warning = False  # 只关注行人检测，去除红绿灯相关变量
    light_color = None
    # COCO (common objects in context中 0代表行人,9代表红绿灯)
    for result in results:
        # 提取当前图像中所有检测目标的类别索引,并转换为Python列表格式
        cls = result.boxes.cls.tolist()

        if len(cls) > 0:
            cls = cls[0]
            # 只处理行人类别（cls == 0），移除红绿灯检测逻辑
            if cls == 0:
    ...
    return person_Warning,light_color  # 只返回行人警告状态
...
def auto_run(image, mqtt_client, pts_middle, pid):
    person_Warning,light_color = yolo_detect(image)
    carspeed = 0 if person_Warning else 18
    ...
    #     print("检测到黄灯,减速,准备停车")
        ...
        print(f"行人检测触发停车,发送车速指令:{carspeed}")
    else:
        print(f"未检测到行人,发送车速指令:{carspeed}")
    ...
    mqtt_client.send_mqtt(json.dumps({"carDirection": steering_angle}))  # 发送控制角度
    ...
    while True:
        try:
            ...
            if 'image' in image:
                ...
                # if 'image' in image:
                image = b64_to_np(image)#base转opencv
