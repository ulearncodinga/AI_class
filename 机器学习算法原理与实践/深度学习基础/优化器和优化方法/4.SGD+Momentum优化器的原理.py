''''''
'''
SGD+Momentum是随机梯度下降（Stochastic Gradient Descent，SGD）的一种变体，它引
入了动量（Momentum）的概念来改进传统的随机梯度下降算法。动量的引入有助于解决SGD
中的  震荡和收敛速度较慢  的问题。可以更好地应对梯度变化和梯度消失问题，从而提高训练模
型的效率和稳定性。
...
def SGD_MoMentum(points,w,b,lr,batch_size):
     global v_w,v_b,momentum
