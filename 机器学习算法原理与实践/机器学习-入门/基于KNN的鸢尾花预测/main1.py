...
#数据可视化,降维 使用t-sne算法进行降维操作,将特征降到二维,方便可视化
tsne = TSNE(n_components=2)
x_tsne = tsne.fit_transform(X_test_scaled)
#将字符串标签,转换为数值标签,方便画图
label_encode = LabelEncoder()
y_test_numeric = label_encode.fit_transform(y_pred)

plt.figure(figsize=(8,6))
scatter = plt.scatter(x_tsne[:,0],x_tsne[:,1],c=y_test_numeric,cmap='viridis')#cmap:颜色映射方案
plt.title('t-tsne Visualization of KNN Predictions')
